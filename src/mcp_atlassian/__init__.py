...
@click.option(
    "--dc-oauth-setup",
    is_flag=True,
    help="Run OAuth 2.0 setup wizard for Jira/Confluence Data Center (3LO, Application Links)",
)
@click.option(
    "--transport",
...
def main(
    ...
    if dc_oauth_setup:
        logger.info("Starting Data Center OAuth 2.0 setup wizard")
        try:
            from .utils.oauth_setup import run_dc_oauth_setup

            sys.exit(run_dc_oauth_setup())
        except ImportError:
            logger.error("Failed to import DC OAuth setup module.")
            sys.exit(1)
