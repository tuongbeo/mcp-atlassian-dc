...
# =============================================================================
# Data Center OAuth 2.0 Setup
# =============================================================================


@dataclass
class DCOAuthSetupArgs:
    """Arguments for the Data Center OAuth 2.0 setup flow."""
...
def run_dc_oauth_flow(args: DCOAuthSetupArgs) -> bool:
    """Run the OAuth 2.0 authorization flow for Jira/Confluence Data Center.

    Uses Authorization Code flow (PKCE optional — DC supports but doesn't require it).
    The OAuthConfig with base_url automatically routes to the DC token endpoint:
      POST {jira_url}/rest/oauth2/latest/token

    Args:
        args: DCOAuthSetupArgs with all required config
    ...
    # OAuthConfig with base_url set → is_data_center = True → uses DC endpoints
    oauth_config = OAuthConfig(
        client_id=args.client_id,
        client_secret=args.client_secret,
        redirect_uri=args.redirect_uri,
    ...
    auth_url = oauth_config.get_authorization_url(state=state)
    logger.info(f"Opening browser for DC authorization at {auth_url}")
    ...
    logger.info("Exchanging authorization code for tokens...")
    success = oauth_config.exchange_code_for_tokens(authorization_code)
    ...
    logger.info("✅ DC OAuth authorization successful!")

    # Print env vars to configure
    print("\n" + "=" * 70)
    print("  SETUP COMPLETE — Copy these environment variables to your .env")
    print("=" * 70)
    print(f"JIRA_URL={args.jira_url}")
    if args.confluence_url:
        print(f"CONFLUENCE_URL={args.confluence_url}")
    print(f"ATLASSIAN_OAUTH_CLIENT_ID={args.client_id}")
    print("ATLASSIAN_OAUTH_CLIENT_SECRET=<your-secret>")
    print(f"ATLASSIAN_OAUTH_REDIRECT_URI={args.redirect_uri}")
    print(f"ATLASSIAN_OAUTH_SCOPE={args.scope}")
    print("=" * 70)
    print(
        f"\nTokens saved to: ~/.mcp-atlassian/oauth-{args.client_id}.json"
    )
    print("The server will auto-refresh tokens before expiry (every ~2h).\n")

    return True


def run_dc_oauth_setup() -> int:
    """Interactive OAuth 2.0 setup wizard for Jira/Confluence Data Center.

    Pre-requisites (run once by Jira DC Admin):
      Administration → Applications → Application Links
      → Create link → External application → Incoming
      → Enable scopes (READ, WRITE) → Copy Client ID + Client Secret

    Returns:
        0 on success, 1 on failure
    """
    print("\n" + "=" * 70)
    print("  MCP Atlassian — Jira Data Center OAuth 2.0 Setup Wizard")
    print("=" * 70)
    print()
    print("Pre-requisites (must be done by Jira DC Admin):")
    print("  1. Go to:  Administration → Applications → Application Links")
    print("  2. Create link → External application → Incoming direction")
    print("  3. Set Redirect URL: http://localhost:8080/callback")
    print("  4. Enable scopes: READ, WRITE")
    print("  5. Copy Client ID and Client Secret below")
    print()

    # Jira URL
    jira_url = _prompt_for_input("Jira DC URL (e.g. https://jira.pila.vn)", "JIRA_URL")
    if not jira_url:
        logger.error("Jira URL is required")
        return 1

    # Confluence URL (optional)
    confluence_url = _prompt_for_input(
        "Confluence DC URL (optional, press Enter to skip)", "CONFLUENCE_URL"
    )

    # Client credentials
    client_id = _prompt_for_input("Client ID", "ATLASSIAN_OAUTH_CLIENT_ID")
    if not client_id:
        logger.error("Client ID is required")
        return 1

    client_secret = _prompt_for_input(
        "Client Secret", "ATLASSIAN_OAUTH_CLIENT_SECRET", is_secret=True
    )
    if not client_secret:
        logger.error("Client Secret is required")
        return 1

    # Redirect URI (default: localhost:8080)
    default_redirect = os.getenv(
        "ATLASSIAN_OAUTH_REDIRECT_URI", "http://localhost:8080/callback"
    )
    redirect_uri = (
        _prompt_for_input("Redirect URI", "ATLASSIAN_OAUTH_REDIRECT_URI")
        or default_redirect
    )

    # Scope (DC uses READ WRITE, not Cloud-style scopes)
    default_scope = os.getenv("ATLASSIAN_OAUTH_SCOPE", "READ WRITE")
    scope = (
        _prompt_for_input("Scope (space-separated)", "ATLASSIAN_OAUTH_SCOPE")
        or default_scope
    )
    ...
    args = DCOAuthSetupArgs(
        jira_url=jira_url,
        client_id=client_id,
        client_secret=client_secret,
        redirect_uri=redirect_uri,
        scope=scope,
        confluence_url=confluence_url if confluence_url else None,
    )

    success = run_dc_oauth_flow(args)
    return 0 if success else 1
